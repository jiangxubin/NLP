...
bool SymSpellCheck::ExpandDictionary(string corpus)
{
	ifstream infile(corpus);
	if(!infile.good()) return false;
	string out_path(corpus.substr(0, 7));
	out_path+="expanded.txt";
	...
	while(getline(infile, line)){
		...
		ss>> key >> count;
		vector<char> data(key.begin(), key.end());
		vector<char> all_upper;
		vector<char> first_upper(key.begin(), key.end());
		if(first_upper[0]>=97&&first_upper[0]<=122){
		    first_upper[0] = char((int)first_upper[0]-32);
		}
		string first_upper_write_string = string(first_upper.begin(), first_upper.end());
		first_upper_write_string += " ";
		first_upper_write_string += to_string(count);
		first_upper_write_string += "\n";
		outfile << first_upper_write_string;
		for(auto iter=data.begin(); iter!=data.end();iter++){
		    if(*iter>=97 && *iter<=122){
                char up = char((int)(*iter)-32);
                all_upper.push_back(up);
		    }
		    else{
		        all_upper.push_back(*iter);
		    }
		}
		string all_upper_write_string = string(all_upper.begin(), all_upper.end());
        all_upper_write_string += " ";
        all_upper_write_string += to_string(count);
        all_upper_write_string += "\n";
		outfile<<all_upper_write_string;
	}
	infile.close();
	outfile.close();
	return true;
